...
def getgitenv(user, date):
   ...
   if elems := re.compile('(.*?)\s+<(.*)>').match(user):
      env += f'export GIT_AUTHOR_NAME="{elems.group(1)}" ;'
      env += f'export GIT_COMMITTER_NAME="{elems.group(1)}" ;'
      env += f'export GIT_AUTHOR_EMAIL="{elems.group(2)}" ;'
      env += f'export GIT_COMMITTER_EMAIL="{elems.group(2)}" ;'
   else:
